...
app.get('/api/admin/documents/:id/pdf', requireAdmin, (req, res) => {
  ...
  const pdf = new PDFDocument({ size: 'A4', margin: 52, info: { Title: `${kind} ${document.number}`, Author: details.owner_name || 'Thibault Dubois' } });
  ...
  pdf.fillColor('#ffffff').font('Helvetica-Bold').fontSize(10).text('Prestation', 62, tableTop + 8);
  pdf.text('Qté', 350, tableTop + 8, { width: 35, align: 'right' });
